...
const processPosts= async(option) => {
  if (option==1){
    ...
    getPosts.forEach(forumPost => {
        ...
        let postOutline = `<div class="card container-fluid w-25" style="background: ${postObject.backgroundColor}" >
      <img src="${postObject.imageURL}" class="card-img-top img-fluid" alt="...">
      <div class="card-body" >
        <h5 class="card-title opensans">Ngày đăng:${new Date(postObject.dayPublished * 1000)}</h5>
        <h5 class="card-title opensans">${postObject.postTitle}</h5>
        <p class="card-text opensans" style="display: -webkit-box; -webkit-line-clamp: 5; line-clamp:5; -webkit-box-orient: vertical; overflow: hidden">${postObject.postContent}</p>
    ...
    if (html !== ""){
      alertText.innerText= "Dưới đấy là những bài luận được đăng mới nhất"
    }
