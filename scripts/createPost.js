...
const processAndSavePosts = () =>{
    ...
    if (!postTitle || !subject || !grade || !postContent ){
        alert("Vui lòng nhập đầy đủ thông tin cho bài viết của bạn!")
        return 
    }
    else if(postContent.length < 10){
        alert("Vui lòng nhập nội dung bài đăng dài hơn 10 ký tự!")
        return
    }
    else{
        if(chooseImage == undefined){
            
            let postDataStructure = {
            ...
            postContent,
            // backgroundColor,
            dayPublished: new Date().getTime(),
            emailAddress: userSession.emailAddress
        }
        localStorage.setItem("postData",JSON.stringify(postDataStructure))
    }
        else{
        ...
        const getpostData = JSON.parse(localStorage.getItem("postData"))
        console.log(getpostData)
            database.collection("EducammunityForumPosts").add(getpostData)
            .then(docReference => {
