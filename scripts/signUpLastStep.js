...
function provideGravatarHash(emailAddress){
                // const crypto = import("crypto")crypto.createHash("sha256").update(email).digest("hex")""nhatdbd@gmail.com
                emailAddress = emailAddress.trim().toLowerCase()
                const gethash = md5(emailAddress)
                return gethash
            }
            
const signInLastStep = async () =>{
    ...
    if (grade < 1 || grade > 12){
        ...
        if (imageOfUser){
            ...
            .then(imageResult => {localStorage.setItem("imageOfUser",JSON.stringify(imageResult.secure_url)), console.log(imageResult.secure_url)})
        }
        else{
            const gethash = provideGravatarHash(savedUserData.emailAddress)
            const avatarURL = `https://0.gravatar.com/avatar/${gethash}?d=identicon`
            localStorage.setItem("imageOfUser",JSON.stringify(avatarURL))
        ...
        savedUserData.imageOfUser= JSON.parse(localStorage.getItem("imageOfUser"))
